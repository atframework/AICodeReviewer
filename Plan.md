...
`<LinkCard>` + 新增“评审输出规范”章节，并修正失效锚点。`custom.css` 增强 4K 全宽（regular
`clamp(45rem,62vw,100rem)`、splash `clamp(50rem,84vw,155rem)`、prose `78ch`、新增 200rem
padding 档与 120/180rem 下 3/4 列卡片网格）。新增 `docs/site/public/favicon.svg`（logo/favicon）。
