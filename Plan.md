...
| 优先级 | 工作 | 本地产物与验收 | 边界 |
| --- | --- | --- | --- |
| P0 | Workspace 多工程规则、动态配置管理与自动迁移 | 按下方设计和阶段计划实现；配置 UI 到下一次任务的完整链路、文件来源只读、所有目标数据库升级均有测试证据 | P0 解析、合并、changeset 与字段清单已有实现和回归测试；passthrough 字段的类型化能力校验仍待补全；P1–P8 未实现，未执行迁移；PostgreSQL store 接线待补 |
...
本项已进入 P0 基础层实现与审查；后续阶段和跨层验收仍待推进。详细资料：
...
| 执行顺序 | 尚待实施的交付 | 完成条件 |
| --- | --- | --- |
| P0–P1 | 补全 passthrough 字段的类型化能力校验、多工程规则、Handlebars 路径与变量目录 | 同规则的不同工程独立运行；未知变量、路径逃逸和歧义可诊断；旧映射语义保留 |
