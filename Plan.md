...
- 当前活跃方向是 M11：可发布到 GitHub Pages 的用户文档站子工程。M11-P1（脚手架）、
  M11-P2（骨架 + 首批核心页）和 M11-P3（全章节双语正文迁移）已完成；下一步是
  M11-P5 启用真实发布链路（需仓库权限）和 M11-P6 打磨（链接检查、配置字段覆盖校验）。
...
- M11-P1（脚手架）、M11-P2（骨架 + 首批核心页）和 M11-P3（全章节双语正文）已完成：
  ...
  独立 docs job（`pnpm docs:build`，含公开/内部内容边界校验），GitHub Pages workflow
  草案就绪，中英双语并行（全部带前缀 `/en/` + `/zh-cn/`），全部章节双语正文已落地。
...
| M11 | 进行中 | `docs/ai/documentation-site-plan.md` | M11-P5 启用真实发布；M11-P6 打磨 |
...
下一本地执行包建议为 M11-P6（打磨）：引入链接检查、配置字段覆盖校验脚本、SEO 与贡献
规则自动化。M11-P5（真实 GitHub Pages 发布）需仓库 Settings > Pages 权限，列入 Backlog。
...
| 文档站 GitHub Pages 线上发布 | M11 | 本地静态构建可先完成；真正发布需要仓库 Pages 设置和 Actions 权限 |
...
- M11 文档站发布将走静态站点构建与 GitHub Pages workflow，不改变 AICR 服务运行时镜像。
