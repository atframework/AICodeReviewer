...
- 已完成的本地优先队列 P0-P12 已移出路线图正文并归档到 `docs/ai/milestones/local-priority-queue.md`；当前重新核对后，暂无线程内可继续推进且不依赖外部系统的新本地执行包；外部验收项见 §8.4。
- M8 观测底座与内置观测首页已基本完成：OTel 已接入 serve 命令、Prometheus metrics 和 run snapshot 已连线、eval CLI 与 CI-safe fixture validation 已接入 root CI；Dashboard 支持 Overview / Projects / Providers / Runs 四个标签，工程面板与 Provider 面板均支持 today/thisWeek/thisMonth/all 时间维度切换。
...
- M10（模型元数据 Catalog / models.dev 集成）已基本交付并归档到 `docs/ai/milestones/M10.md`；SQLite、memory 与 Redis 结构化后端可用；Redis 后端复用 `storage.cache.redis`，本地合同层已交付，详见 §3.13、`docs/ai/architecture.md` §3.13。
- 公网正式环境稳定性修复已落地并归档到 AGENTS.md pitfall：沙箱进程树回收 + 容器 `--init`（#49/#50）、issue triage 按事件 provider 族门控（#51）、git/P4/SVN `fetchExtraContext` 回退补拉（#52）、push 全零 SHA 跳过（#53）。
- 直接 push 到主干的 review 与 issue 创建链路已核查正确：两个 GitHub trigger 订阅 `[pull_request, push, issues]`，全局 `outputs.routes` 按 `target_kind` 路由 push → issue 通道，orchestrator `publishSummary` 始终带 problems 驱动 `reconcileProblems`；详细合同见 §3.1、`docs/ai/architecture.md` §3.1。
...
- 成本估算按 token 类别（非缓存输入 / 缓存命中 / 缓存写入 / 输出）套 catalog 真实价格，provider extractor 归一化原生缓存 token；详见 §8.2 P11。
...
| M8 | 基本完成 | `docs/ai/milestones/M8.md` | CI 已验证 eval fixtures；真实 LLM benchmark 仍在 Backlog |
| M9 | 基本完成 | `docs/ai/milestones/M9.md` | 不进入版本 bump / git tag |
| M10 | 基本完成 | `docs/ai/milestones/M10.md` | Redis 结构化缓存后端本地合同层已归档；真实外部 Redis smoke/e2e 可按需放入 Backlog |

### 8.2 当前执行包

P12（CI-safe eval fixture validation）已完成并归档：`aicr eval --validate-only` 可在不加载 config/LLM 的情况下校验 `eval/*.json` fixture 合同，root `pnpm ci` 在 build 后运行该校验；真实 LLM benchmark 仍作为 §8.4 外部验收项。当前暂无新的本地闭环执行包；发布 tag 暂不进入计划。
...
P0-P12 已完成并归档到 `docs/ai/milestones/local-priority-queue.md`。当前无待执行本地项。最新一项 P12 为 CI-safe eval fixture validation（见 §8.2），前一项 P11 为按 token 类别的缓存成本估算。
...
| CI 真实 LLM eval benchmark | M8 | root CI 已运行 `aicr eval --validate-only` 校验 fixture；真正调用 LLM 的 benchmark 仍需 CI secrets / pipeline 权限，延后扩展 |
