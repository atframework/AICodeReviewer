...
| 优先级 | 工作 | 本地产物与验收 | 边界 |
| --- | --- | --- | --- |
| P1 | 自动批次逐目标发布恢复 | 先梳理 publisher 能力矩阵；设计持久目标回执及状态转换；覆盖部分成功、响应丢失、租约过期与重入调用次数 | 当前 completed 检查点仅恢复本地记账；其他重放接受重复发布风险。终态自动恢复一次后跳过，管理端支持 Retry；逐目标远端对账另行验收 |
...
| 场景 | 已有本地证据 | 仍需的条件与验收 |
| --- | --- | --- |
| 跨 workspace 并发与历史保留部署验收 | 共享执行名额、P4 阻塞期间 GitHub Retry、三后端终态清理及统计保留回归 | 在目标部署核对 `queue.workers` 生效值、自然任务并发和重启恢复；核对 YAML/数据库保留策略与重置，确认既有汇总不变。本地测试不代表公网事件重放验收 |
