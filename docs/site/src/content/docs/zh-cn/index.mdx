...
评审是 **precision-first** 的：agent 只报告经过验证、可操作的问题，summary 只框定
这些问题，而不会罗列"检查过但没发现异常"的部分。当没有可操作问题时，评审以显式
skip 收尾，而不是发布一篇空洞的表扬式 summary。

<Aside type="tip" title="渲染与安全保证">
  发现被渲染为规范 Markdown——IM 机器人收到真正的表格和代码块，而不是原始
  stdout。 Secret 会从
  prompt、日志和每个输出边界脱敏，评审也被引导去推理常见漏洞类别 （其中包括
  OWASP Top 10）。
</Aside>
