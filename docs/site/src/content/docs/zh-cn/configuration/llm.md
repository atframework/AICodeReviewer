...
启用数据库配置后，目录设置、模型链和 triage 链对下一次接收的任务生效。
每个配置快照固定所配置模型的解析元数据，重启后仍保持；共享缓存刷新不会改变
旧任务的 fallback 或 summary 模型。

发布预算变更会保留本进程累计的日费用。下一次模型调用按已报告费用检查新上限；
一次响应可能使费用超过上限，后续调用才会被阻止，因此它不是远端计费硬封顶。
日费用在内存中累计，进程重启后重置。
