...
closes with an intentional skip rather than a praise-filled summary.
...
<Aside type="tip" title="Rendering & security guarantees">
  ...
  code blocks, not raw stdout. Secrets are scrubbed from prompts, logs, and
  every output boundary, and reviews are guided to reason about common
  vulnerability classes (the OWASP Top 10 among them).
</Aside>
