...
`aicr.report_problem` does not accept agent-supplied attribution. When author or revision context is needed for analysis, the agent must call `aicr.try_blame`; AICR validates the request through event metadata, provider APIs, or the configured VCS adapter before feeding attribution back into a follow-up pass. Attribution remains advisory context for review reasoning and must not become part of the problem fingerprint.
...
- `consolidated`:
  ...
  - Retained-only partial reconciliation refreshes the severity/count/location prefix but preserves the existing descriptive suffix, so an out-of-scope placeholder never replaces the issue topic.
...
- Use `aicr.fetch_more_context` only for bounded, justified source-context gaps.
