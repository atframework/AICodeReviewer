...
状态：P0–P8 历史交付见 M17–M22；复审修复及剩余跨版本验收见
[M23](../../ai/milestones/M23.md)。
修复与验证见 [M19](../../ai/milestones/M19.md)，P6 交付与门禁证据见
[M20](../../ai/milestones/M20.md)，P7 证据矩阵与门禁见
[M21](../../ai/milestones/M21.md)。以下勾选以实际证据为准。设计合同见
...
- [x] SQLite/Redis/PostgreSQL 配置后端升级或重启后消费真实签名事件；`post-migration-consumption.test.ts` 断言旧 SQL 数据保留、新版本实际模型请求，专用 Redis AOF 子进程重启后消费原 revision。
- [x] 多工程路径、模型与输出归属：`config-e2e-publish-review.test.ts` E04/E05 补齐多 Git profile、GitLab subgroup/fork、实际输出请求与凭据；P4/SVN 真实本地服务证据见 M21。
- [x] 两副本读同配置源：`replica-fault-matrix` 与 `replica-process-matrix` 覆盖同版本双进程 adoption、commit 后 install 前屏障 SIGKILL、operationId 去重、PG/Redis 断连重启。此项不证明旧程序兼容。
- [x] UI 运行中修改 provider URL、模型、route 和 agent；`ui-run-isolation.spec.ts` 断言新建数据库 workspace/group 生效、旧/新 CLI bundle 与 model 参数、两个完成事件的 revision、输出目标与次数。旧 receipt 组批另有 H08/H10/R08 证据。
...
- [x] UI → API → DB → Runtime → 输出组合由 `ui-run-isolation.spec.ts` 覆盖；使用 build/tmp 内真实 Git clone、受控 agent CLI 和 HTTP 输出服务。
...
- [x] `example/config.yaml` 将本轮 planned 注释转为 schema 可执行配置，变量/helper 目录生成校验；`example/README.md` 增加 UI 管理、路由预览、升级命令和冲突优先级。
- [x] 更新 `docs/site/src/content/docs/{en,zh-cn}/configuration/{overview,llm,agent,outputs,storage,queue}.md`、`start/dashboard.md`、`reference/{config-fields,template-variables,cli}.md`、`integrations/{vcs-providers,agent-adapters,output-channels}.md` 和部署运维页，两种语言同次同步。
- [x] 更新 `docs/ai/architecture.md` 的 workspace、运行时、配置、存储合同，决策写入 `docs/ai/decisions.md`；源码链接和字段目录保持真实。
- [x] 更新 `.agents/skills/agent-runtime-integration/`、`output-channel-contracts/`、`plan-implementation-audit/` 中实际受影响的运行时/路由/版本验证规则；如出现已修复可复发错误，通过 `AGENTS.known-pitfalls.md` 导航写入对应专题，不复制整份设计到根提示词。
- [x] 审查 `prompts/system/code-reviewer.system.md` 是否仍写死旧 source/agent 布局；只在需要改变实际 agent 运行指导时更新，不把数据库管理操作灌入 review prompt。
- [x] Linux 原生文件系统运行 `pnpm run ci`；Windows 完整顺序门禁、浏览器和文档站通过。两端均发现 169 个测试文件；Windows 真实服务零跳过，Linux 原生服务条件跳过，数量及环境见 M23。
- [x] 最终 diff、Markdown 发现、链接、示例字段及双语一致性检查通过；实测数量和 backend 版本见 M23。
