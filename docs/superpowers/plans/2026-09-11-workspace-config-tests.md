...
P0–P8 历史交付及复审证据见 [M23](../../ai/milestones/M23.md)；M17/M18 跨版本
验收仍未完成。阶段记录见 [M19](../../ai/milestones/M19.md)、
...
### 7.1 E01–E10 证据映射（2026-09-15 复审）

| ID | 组合证据（文件 → 覆盖的断言） |
| --- | --- |
| E01 | `ui-run-isolation.spec.ts`：UI 创建 provider/group/workspace/channel/route，预览布局播种真实 Git，签名 webhook 到真实 CLI 审查；断言模型参数、bundle URL、完成 revision 和输出请求。API 全链另见 `config-e2e-publish-review.test.ts` |
| E02 | 同一浏览器流程阻塞 run1，经 UI 修改 provider URL/model/route/agent，run1 全旧、run2 全新；精确比对 kilo/opencode bundle、argv、完成 revision、snapshot 与目标/次数 |
...
| E04 | `config-e2e-publish-review.test.ts` E04：同规则多 Git 工程的目录、最终模型、所属 host/token 和输出次数；`webhook-repo-attribution` 另测 owner/host/大小写身份 |
| E05 | `config-e2e-publish-review.test.ts` E05：两 host 的 subgroup/fork MR，经鉴权、配置 pin、真实 orchestration 到编码完整项目路径的出站请求；精确 private-token、MR iid、source/target 身份和次数。另测 Note Hook、缺失/非法 iid |
...
| E08 | `post-migration-consumption.test.ts`：SQLite/PG 旧 schema 业务数据保留、配置升级后实际新事件 review；Redis 专用 AOF 子进程 SIGKILL/重启后消费持久 revision。Redis 当前无历史结构迁移步骤，不能声称已测不存在的升级 |
| E09 | `replica-process-matrix.test.ts`：同版本独立子进程、持久 commit 后 install 前显式屏障 SIGKILL、恢复与 operationId 去重；真实 PG/Redis 断连/重启。双版本 reader/writer 仍未验收 |
| E10 | `tests/browser/config-ui.spec.ts` 桌面全矩阵与窄屏；含 E01/E02 的完整浏览器门禁结果见 M23 |

完整测试数量与环境见 [M23](../../ai/milestones/M23.md)；本地服务通过不等于目标
部署 ACL/TLS 验收。M17/M18 缺少目标旧版本与兼容协议，不能用 E09 替代。
...
Linux 使用 `pnpm run ci`。修改 docs/site 时另跑 `pnpm docs:check`、`pnpm docs:build`。新增正式 browser suite 和真实 PostgreSQL/Redis acceptance 在 CI/文档中有独立可复现命令，命令确定后再写进已支持 CLI 文档。
