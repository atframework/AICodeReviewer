...
5. **输出协议化**：最终 review 结论必须经 `aicr.publish_finding` /
...
| PR-Agent | system+user 模板，按 diff hunk 组织 | YAML schema，finding 数量受限 | 空 issue 列表可接受 | 聚焦新增行，要求高置信 | 基于 diff、票据、extra instructions | **采纳**：finding cap、结构化输出、高置信阈值 |
...
- 以 diff hunk 为主要输入单位。
- 明确只关注本次 PR 引入的问题。
- 输出有严格 schema，并限制最大 findings 数量。
- 允许注入 `extra_instructions`，但要求具体、清晰、简洁。

**采纳项**：

- 默认 review 只看当前变更引入的问题。
- 默认只输出少量高价值 findings，而不是“能说的都说”。
- 输出结构要可程序化消费。
...
4. **finding 控量**：默认只输出最关键、最高置信的一小组问题。
5. **静默优先**：无可执行问题则 `skip`。
...
- **按需 recall**：当某个 finding 需要更长背景时，再调 `aicr.fetch_more_context` 或
  `aicr.recall_skill`。
...
| `low` | 小范围风险、次要性能/维护性问题，但有明确未来故障面 | 仅在 finding 预算有余时发布 |
...
- `info`：默认不用 line-level finding 占位。

### 默认 finding 预算

- 默认最多保留 **5 条** line-level findings；
...
9. **零 finding 场景**：确认输出 `aicr.skip(reason="lgtm")` 而非空洞 praise；
...
3. 约束 agent 只通过 `aicr.publish_*` 输出正式结论。
4. 要求 finding 必须具备：
   - 具体问题；
   - 现实触发场景；
   - 影响说明；
   - 必要时的修复方向；
   - 不确定性说明。
5. 要求无 actionable finding 时调用 `aicr.skip(reason="lgtm")`。
...
9. 定义默认 finding 预算与 summary 行为，避免 review 结果膨胀成冗长评论流。
