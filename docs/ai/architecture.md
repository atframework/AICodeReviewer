...
- P4 需要支持：
  ...
  - SSL 指纹未信任（首次连接提示或 `P4PORT IDENTIFICATION HAS CHANGED`）时自动 `p4 trust -y` 并重试一次；指纹已存在但不匹配（密钥轮换）时 `p4 trust -y` 会拒绝替换，适配器退到 `p4 trust -y -f` 强制替换。`p4 login` 遇到 trust 提示走同一修复路径。trust 错误在瞬时 IO 分类中仍属永久错误，不靠重试分类器兜底。
  - 配置的 client workspace 在服务器上被删除（`Client '<name>' unknown - use 'client' command to create it.`）时，适配器用最小只读 spec 经 `p4 client -i` 自动重建（Root 指向 AICR workspace 目录，View 映射 `<depot_path>/...`），每条命令最多重建一次并重试；AICR 不执行 `p4 sync`，client 记录只需存在即可。重建失败按原错误路径降级（listChanges/diff 落空并告警）。
  - basename 级别 glob 过滤
...
- `agent.context_compaction`（默认启用）将各 agent 原生的 auto-compaction 设置注入生成的配置：
  ...
  - **pi**：只注入 `settings.json` 的 `compaction.enabled`；触发时机与阈值由 CLI 管理，manifest 记为 delegated。
  - **oh-my-pi**：注入 `config.yml` 的 `compaction.enabled` / `thresholdPercent`，manifest 记为 injected。
...
- P1b 起 git 系 webhook 的运行时匹配已接线:`packages/core/src/config-resolution.ts` 定义
  ...
  `manual.*` 仅来自受信 CLI。每个 run 独立 sandbox/HOME/XDG/临时目录，P4 client 按
  主机与运行根隔离，reflection 按 instance ID 保存。`WorkspaceLayout.policyRoot` 指向
  definition 的旧策略目录，作为 templates/AGENTS/skills 的只读回退，实例/源码资源优先。
- `workspaces` 采用三段式：
  - `workspaces.cache`
  - `workspaces.defaults`
