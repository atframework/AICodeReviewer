...
这份归档记录此前从 `Plan.md` 拆出的 P0-P12 本地闭环任务。它们的共同标准是：不依赖真实 GitLab / SVN / Redis / Kubernetes / CI 权限，可通过本地单元测试、集成测试、typecheck、build 或 markdownlint 验证。
...
- 新的本地优先项仍应先写回 `Plan.md` §8.2（当前执行包），完成后再归档到对应 milestone 或本文件。
