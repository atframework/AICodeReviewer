...
| D24 | 提交归因契约 | attribution 必须来自事件、provider API 或只读 VCS 工具验证，不得猜测。VCS 层归因通过**可选** `VcsAdapter.fetchAttribution` 提供（best-effort，缺失返回 `not_found`/`partial`，不污染 fingerprint），`aicr.try_blame` MCP 工具在落地测试前不对外宣传。 | `docs/ai/architecture.md` §3.2、§3.9.2、`packages/vcs/src/{contracts,git,p4,svn,attribution}.ts` |
...
| D30 | Reflection memory 存储 | `reflectionMemory` 表按 workspace 隔离，支持 TTL 过期、条目上限压缩和 `occurrence_count` 计数。review 开始前读取并注入 `memoryHints`，review 完成后写入 light reflections；`thorough` mode 的最小跨 run 聚合已基于重复 category 落地。repo 约定学习和跨 workspace 知识迁移仍需范围/脱敏策略决策。 | `docs/ai/architecture.md` §3.12、`packages/store/src/reflection.ts`、`packages/core/src/reflection-extractor.ts` |
