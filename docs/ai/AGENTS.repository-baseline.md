...
1. **Config schema gaps**: `packages/core/src/config.ts` Zod schemas must include all fields from Plan.md §3.10 — `compression`, `llm.fallback_chain`, `llm.retry`, `llm.budget`, `llm.per_provider_overrides`, `queue.workers`, `queue.rate_limit`, `queue.retry`, `queue.dead_letter`, `review.reflection.memory`, `workspaces.defaults.agent`, `outputs.channels[].mention_fallback`, `outputs.routes`.
2. **Store schema columns**: `packages/store/src/schema.ts` must include `triggerName`, `provider`, `providerModel` per Plan.md §3.11.
