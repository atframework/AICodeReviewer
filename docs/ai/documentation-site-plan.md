...
- ✅ 首页改用 splash `hero`（title/tagline/actions + 内联 SVG logo）、`<Steps>` 工作流、
  ...
  会导致构建失败）。
- ✅ 4K 全宽（全出血）：`custom.css` 移除内容列 max-width 上限
  （`--sl-content-width: 100%`，与 Starlight 打印样式做法一致），regular 页填满 sidebar
  与 TOC 之间、splash 页填满视口；渐进 padding（2/3/4/6rem）保留留白；在 ≥176/224/300rem
  （≈ 2.5K/4K/5K）按比例放大 root 字号，在无宽度上限下保持文本可读（媒体查询 rem 基于初始
  16px，阈值不受影响，1080p/1440p 不受影响）；prose 通过 `--aicr-prose-measure`
  （默认 `none`）可选恢复阅读宽度；splash 页 hero SVG 流式尺寸与 120/180rem 下 3/4 列卡片网格。
