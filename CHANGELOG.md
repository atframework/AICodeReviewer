...
- `review.pull_request.include_target_branches`: receive-side target (base) branch allowlist for PR/MR analysis, layered global → workspace defaults → instance with wholesale replacement (`[]` clears back to all branches). `ReviewEvent` gains `targetBranch`, mapped from Gitea/GitHub `pull_request.base.ref`, GitLab `target_branch`, and comment-command PR-detail `base.ref`; PRs/MRs targeting unlisted branches are ignored at receive time (webhook log decision `ignored`, reason `target_branch_not_watched`) with no run produced, events with an unknown target branch fail open, and push/issue/manual flows are never filtered.
- `review.auto_commit.include_branches`: receive-side branch allowlist for automatic commit events (push / P4 change-commit / SVN post-commit), layered global → workspace defaults → instance with wholesale replacement (`[]` clears back to all branches). Pushes to unlisted branches are ignored before receipt persistence (webhook log decision `ignored`, reason `branch_not_watched`); PR/MR/comment flows are unaffected and branchless P4/SVN hooks bypass the filter. The resolved list stays out of `policyVersion`, so sealed receipts remain valid.
...
- GitHub/Gitea managed issues resolve head-commit authors before the retained push pusher, including automatic Git batches with only name/email metadata. Fixed Gitea's `/git/commits/{sha}` lookup and empty webhook usernames, preserved email blacklists, and cached concurrent lookups. Creation retries without assignees only for explicit 422 assignment validation errors. GitLab issue creation/assignment remains unimplemented (`gitlab_mr_review` publishes discussions/notes).
- Dashboard lands on the Overview tab again instead of the Live worker view; Live is now the second tab and only polls while selected.
- GitLab `Push Hook`/`git_push` now use automatic commit branch filtering and receipt persistence; all-zero branch creation/deletion ranges are ignored consistently across Git providers. PR webhooks reject malformed `base.ref` values instead of treating them as unknown targets. Configuration example checks now include standalone PR policies.
...
- **M15**: Auto-commit metadata expansion for git receipts failed terminally in production (`EACCES: permission denied, rmdir '/app'`). The scheduler's metadata adapter cloned into the process working directory (read-only `/app` inside the runtime image) instead of the per-workspace source root, fetched only once per cached adapter, and never resolved GitHub App installation tokens. The adapter now clones under `workspaces/<workspaceId>/source/<repo>`, re-fetches every sync (`alwaysFetch`), and re-resolves tokens per sync (`tokenProvider`). The scheduler adapter cache is additionally keyed by workspaceId so two workspaces watching the same repo never share a clone directory. Affected production receipts can be recovered by clearing `metadata_terminal_error`/`metadata_attempts` so the scheduler re-expands them.
