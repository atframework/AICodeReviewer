...
4. 制定小步可验证方案。
5. 修改代码、测试、文档和示例。
6. 最后一次修改后，按仓库验证顺序运行全部适用检查，并确认工具实际发现了预期文件或测试。
...
| 顺序 | baseURL selector                                 | token selector                                 | 模型              |
| ---- | ------------------------------------------------ | ---------------------------------------------- | ----------------- |
| 1    | `.llm.provider.zhipu.baseURL`                    | `.llm.provider.zhipu.token`                    | `glm-5.3-flash`   |
