...
const GIT_SECRET = "browser-dummy-git-secret";
...
test("P7: UI route/agent changes mid-run fully isolate the old and new runs", async ({ page, request }) => {
  ...
  // Route r-iso: workspace default-project, pull_request on git-main, file
  // default model chain, summary → c-iso-a (channels must be published first
  ...
  await setSelectField(drawer, "route:workspace", "default-project");
  await (await multiselectRow(drawer, "route:match.triggers")).getByRole("checkbox", { name: "git-main", exact: true }).check();
  await (await multiselectRow(drawer, "route:match.target_kinds")).getByRole("checkbox", { name: "pull_request", exact: true }).check();
  await setSelectField(drawer, "route:analysis.model_chain", "default");
  ...
  // (rev head0+2 at run 1 admission, rev head0+4 at run 2 admission) persisted
  ...
  await expect.poll(async () => {
    ...
    try {
      return db.prepare("SELECT COUNT(*) AS n FROM review_runs WHERE workspace_id = 'default-project' AND status = 'succeeded'").all() as { n: number }[];
    } finally {
      db.close();
    }
  ...
  try {
    ...
    expect(runs).toEqual([
      { status: "succeeded", trigger_name: "git-main", head_sha: shas.headSha },
      { status: "succeeded", trigger_name: "git-main", head_sha: shas.headSha },
    ]);
    ...
    expect(revisions).toContain(head0 + 4);
    expect(new Set(snapshots.map((row) => row.id)).size).toBe(snapshots.length);
  } finally {
    db.close();
  }
  expect(await apiStatusRevision(request, token)).toBe(head0 + 4);
