...
| Build RUN steps (e.g. `pnpm install`) fail with `ECONNREFUSED` to the target registry while the host itself can reach it | The auto-detected build proxy listens on `*:3128`, but its ACL refuses the container bridge subnet; `--http-proxy=true` passes the dead proxy into every RUN | Current `deploy.sh` normalizes a wildcard listener to `http://127.0.0.1:3128` and builds with `--network=host`, which always reaches the proxy. On an older script, export `HTTP_PROXY=http://127.0.0.1:3128` (and `HTTPS_PROXY`) before running it. Verify reachability with a throwaway `podman run --rm --entrypoint /bin/sh -e HTTPS_PROXY=... <image> -c 'curl ...'` probe |
