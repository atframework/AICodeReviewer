...
2. **Preserve per-channel policy semantics**
   ...
   - Managed problem issue lifecycle listing must honor `review.problem_issue.max_recent_issues` (default 30, global → workspace override, range 1..200, open issues only, paginated up to the cap) before closing or deleting stale issues. Gitea/Forgejo may clamp `limit` below the requested value through `MAX_RESPONSE_ITEMS`; treat a short page as terminal only when its `Link` header has no `rel="next"`.
...
4. **Keep docs, examples, and tests aligned**
   ...
   - If resolved issue rendering changes, test the `<!-- aicr:problem-meta=BASE64_JSON -->` marker roundtrip (including the original diagnostic needed by lifecycle analysis), readable title rendering (`[SEVERITY] category — file:line`), legacy no-metadata fail-closed behavior without raw fingerprints, model-confirmed versus retained candidates, and anchor ID generation for both open and resolved items.
