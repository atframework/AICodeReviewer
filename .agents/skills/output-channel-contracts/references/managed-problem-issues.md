...
- Production bootstrap injects one cached resolution analyzer per review and uses the model/client resolved from `llm.triage_fallback_chain`; an absent or empty chain must reuse the code-analysis model/client. Duplicate cleanup remains deterministic because it reconciles managed identity, not code resolution.
