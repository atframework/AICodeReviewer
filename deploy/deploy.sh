...
# When true, manage the container as a systemd (quadlet) service generated by
# podlet instead of a plain `podman run -d`: systemd restarts it after crashes
# and starts it at boot (user lingering required for rootless). Used on the
# public production host. podlet/quadlet is podman-only: with any other engine
# the script warns and falls back to `<engine> run -d`.
AICR_ENABLE_SYSTEMD="${AICR_ENABLE_SYSTEMD:-false}"

# When true, start the container with docker-compose (`<engine> compose`,
# `docker compose`, or `docker-compose`, in that discovery order) using a
# generated $DEPLOY_DIR/docker-compose.yaml with `restart: unless-stopped`
# keepalive. Disabled by default. Mutually exclusive with AICR_ENABLE_SYSTEMD;
# without a compose CLI the script warns and falls back to `<engine> run -d`.
AICR_ENABLE_COMPOSE="${AICR_ENABLE_COMPOSE:-false}"
COMPOSE_FILE="$DEPLOY_DIR/docker-compose.yaml"

if [ "$AICR_ENABLE_SYSTEMD" = "true" ] && [ "$AICR_ENABLE_COMPOSE" = "true" ]; then
  echo "ERROR: AICR_ENABLE_SYSTEMD and AICR_ENABLE_COMPOSE are mutually exclusive; pick one start mode." >&2
  exit 1
fi
if [ "$AICR_ENABLE_SYSTEMD" = "true" ] && [ "$ENGINE_BASENAME" != "podman" ]; then
  echo "WARNING: AICR_ENABLE_SYSTEMD=true requires the podman engine (podlet/quadlet); falling back to '$ENGINE_BASENAME run -d'." >&2
  AICR_ENABLE_SYSTEMD=false
fi
...
# ---------------------------------------------------------------------------
# docker-compose start mode (AICR_ENABLE_COMPOSE=true)
# ---------------------------------------------------------------------------
# Compose CLI discovery, resolved once so the stop and start phases agree.
# Engine-native first (covers both `docker compose` and `podman compose`),
# then the docker plugin, then the legacy docker-compose binary.
COMPOSE_CMD=()
resolve_compose_cmd() {
  if "$ENGINE_CMD" compose version >/dev/null 2>&1; then
    COMPOSE_CMD=("$ENGINE_CMD" "${ENGINE_ARGS[@]}" compose)
    return 0
  fi
...
# Print the values paired with a flag token (-e, -v, --security-opt) from a
# flat args array, one per line, for YAML translation.
print_flag_values() {
  local flag="$1"
  shift
  local take=0 token
...
# Render $COMPOSE_FILE from the same structured pieces that build RUN_ARGS so
# the two start paths cannot drift apart. Regenerated on every deploy.
write_compose_file() {
  local file="$1"
  {
    echo "# Generated by deploy.sh (AICR_ENABLE_COMPOSE=true); overwritten on every deploy."
    echo "services:"
    echo "  ${CONTAINER_NAME}:"
    ...
    echo "    environment:"
    {
      print_flag_values -e "${STATIC_ENV_ARGS[@]}"
      print_flag_values -e "${SANDBOX_ENV_ARGS[@]}"
      print_flag_values -e "${PROXY_ENV_ARGS[@]}"
    } | sed 's/^/      - /'
    echo "    volumes:"
    echo "      - ${DEPLOY_DIR}/config.yaml:/app/config.yaml:ro"
    echo "      - ${DEPLOY_DIR}/data/workspaces:/app/workspaces"
    echo "      - ${DEPLOY_DIR}/data/db:/app/data"
    echo "      - ${DEPLOY_DIR}/data/logs:/app/logs"
    if [ "${#SANDBOX_MOUNT_ARGS[@]}" -gt 0 ]; then
      print_flag_values -v "${SANDBOX_MOUNT_ARGS[@]}" | sed 's/^/      - /'
    fi
    if [ "${#SANDBOX_SECURITY_ARGS[@]}" -gt 0 ]; then
      echo "    security_opt:"
      print_flag_values --security-opt "${SANDBOX_SECURITY_ARGS[@]}" | sed 's/^/      - /'
    fi
    local i key val
    for ((i = 0; i + 1 < ${#USERNS_ARGS[@]}; i += 2)); do
      key="${USERNS_ARGS[i]}"
      val="${USERNS_ARGS[i + 1]}"
      case "$key" in
        --userns)
          echo "    userns_mode: $val"
          ;;
        --group-add)
          echo "    group_add:"
          echo "      - $val"
    ...
    echo "    healthcheck:"
    echo "      test: [\"CMD\", \"node\", \"-e\", \"fetch('http://localhost:${CONTAINER_PORT}/healthz').then(r=>{if(!r.ok)process.exit(1)})\"]"
    echo "      interval: 30s"
    echo "      timeout: 5s"
    echo "      retries: 3"
...
if [ "$AICR_ENABLE_COMPOSE" = "true" ]; then
  if resolve_compose_cmd; then
    echo "=== Compose mode: ${COMPOSE_CMD[*]} with $COMPOSE_FILE ==="
  else
    echo "WARNING: AICR_ENABLE_COMPOSE=true but no compose CLI found ('$ENGINE_BASENAME compose', 'docker compose', docker-compose); falling back to '$ENGINE_BASENAME run -d'." >&2
    AICR_ENABLE_COMPOSE=false
  fi
fi
...
if [ "$AICR_ENABLE_COMPOSE" = "true" ]; then
  "${COMPOSE_CMD[@]}" -p "$CONTAINER_NAME" -f "$COMPOSE_FILE" down --remove-orphans 2>/dev/null || true
fi
...
if [ "$AICR_ENABLE_SYSTEMD" = "true" ]; then
  # Reached only with the podman engine; non-podman engines were normalized to
  # the plain run -d path near the top of this script.
  # systemctl --user needs a runtime bus in non-login SSH sessions.
  export XDG_RUNTIME_DIR="${XDG_RUNTIME_DIR:-/run/user/$(id -u)}"
  export DBUS_SESSION_BUS_ADDRESS="${DBUS_SESSION_BUS_ADDRESS:-unix:path=${XDG_RUNTIME_DIR}/bus}"
  if [ "$(id -un)" = "root" ]; then
  ...
  if command -v podlet >/dev/null 2>&1; then
    ...
    if ! "$ENGINE_CMD" "${ENGINE_ARGS[@]}" image inspect "$PODLET_IMAGE_URL" >/dev/null 2>&1; then
      echo "=== Pulling podlet image ==="
      run_with_build_proxy "$ENGINE_CMD" "${ENGINE_ARGS[@]}" pull "$PODLET_IMAGE_URL"
    fi
    PODLET_RUN=("$ENGINE_CMD" "${ENGINE_ARGS[@]}" run --rm "$PODLET_IMAGE_URL")
  fi

  # --install writes the quadlet; tee keeps the same content at the canonical
  # path (host convention, see /home/tools/setup/traefik/create-traefik-pod.sh).
  # --restart unless-stopped maps to Restart=always in the generated service;
  # without it podlet emits Restart=no and crash recovery is lost.
  echo "=== Generating quadlet ${SYSTEMD_CONTAINER_DIR}/${CONTAINER_NAME}.container ==="
  "${PODLET_RUN[@]}" \
    --install --wanted-by default.target --wants network-online.target --after network-online.target \
    podman run "${RUN_ARGS[@]}" \
    "$IMAGE_NAME" | tee -p "$SYSTEMD_CONTAINER_DIR/${CONTAINER_NAME}.container"

  "${SYSTEMCTL[@]}" daemon-reload
  "${SYSTEMCTL[@]}" start "${CONTAINER_NAME}.service"
elif [ "$AICR_ENABLE_COMPOSE" = "true" ]; then
  echo "=== Generating compose file $COMPOSE_FILE ==="
  write_compose_file "$COMPOSE_FILE"
  "${COMPOSE_CMD[@]}" -p "$CONTAINER_NAME" -f "$COMPOSE_FILE" up -d
else
  "$ENGINE_CMD" "${ENGINE_ARGS[@]}" run -d "${RUN_ARGS[@]}" "$IMAGE_NAME"
fi
