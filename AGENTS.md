...
- For every task, research the current source of truth in depth before choosing a solution: inspect relevant code, config, tests, and existing decisions, and verify external behavior against current authoritative sources. Do not guess or present unverified claims as facts. Then state an evidence-backed plan with assumptions, tradeoffs, affected surfaces, and validation before implementation; ask when unresolved ambiguity changes the implementation.
...
- When changes affect config shape, agent adapters, MCP tool contracts, output rendering, deployment behavior, or public workflow, update the matching `Plan.md` roadmap summary, relevant `docs/` modules, AI-agent prompts and skills, `example/config.yaml`, and `example/README.md` entries in the same change where affected; verify guidance against the implementation and current best practices, or explicitly state why no supporting update is needed.
