...
31. **Review deduplication key must use branch over SHA**: `buildDedupKey` in `packages/server/src/review-deduplicator.ts` must prioritize `reviewEvent.branch` over `headSha` or `baseSha` so that multiple `/aicr review` commands on the same PR/MR are deduplicated even if new commits have been pushed in between.
