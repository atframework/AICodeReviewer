...
describe("review deferrals", () => {
  it("creates the review_deferrals table via migrations", () => {
  ...
  it("upserts and lists pending deferrals ordered by not_before", () => {
    upsertReviewDeferral(store, deferral({ dedupKey: "b", notBefore: new Date(20_000) }));
    upsertReviewDeferral(store, deferral({ dedupKey: "a", notBefore: new Date(10_000) }));
  ...
  it("replaces the envelope on conflict but never moves not_before earlier", () => {
    upsertReviewDeferral(store, deferral({ notBefore: new Date(10_000) }));
    upsertReviewDeferral(store, deferral({
      reviewEvent: JSON.stringify({ targetKind: "pull_request", headSha: "newer" }),
      notBefore: new Date(5_000),
    }));

    let pending = listPendingReviewDeferrals(store);
    expect(pending).toHaveLength(1);
    expect(pending[0]!.reviewEvent).toContain("newer");
    expect(pending[0]!.notBefore.getTime()).toBe(10_000);

    upsertReviewDeferral(store, deferral({ notBefore: new Date(30_000) }));
    pending = listPendingReviewDeferrals(store);
    expect(pending[0]!.notBefore.getTime()).toBe(30_000);
  });

  it("claims a pending deferral atomically and refuses a second claim", () => {
    upsertReviewDeferral(store, deferral());

    const claimed = claimReviewDeferral(store, "key-1");
    expect(claimed).toBeDefined();
    expect(claimed!.status).toBe("claimed");
    expect(claimed!.attempts).toBe(1);

    expect(claimReviewDeferral(store, "key-1")).toBeUndefined();
    expect(listPendingReviewDeferrals(store)).toHaveLength(0);
  });

  it("resets claimed rows back to pending for startup recovery", () => {
    upsertReviewDeferral(store, deferral());
    expect(claimReviewDeferral(store, "key-1")).toBeDefined();

    const reset = resetClaimedReviewDeferrals(store);
    expect(reset).toBe(1);
    const pending = listPendingReviewDeferrals(store);
    expect(pending).toHaveLength(1);
  ...
  it("deletes a deferral once execution owns the outcome", () => {
    upsertReviewDeferral(store, deferral());
    deleteReviewDeferral(store, "key-1");
    expect(listPendingReviewDeferrals(store)).toHaveLength(0);
    expect(claimReviewDeferral(store, "key-1")).toBeUndefined();
  });
});
